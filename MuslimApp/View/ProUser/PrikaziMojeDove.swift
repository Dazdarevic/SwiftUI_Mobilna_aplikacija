...
struct PrikaziMojeDove: View {
            ...
            ScrollView {
                VStack(alignment: .leading) {
                    ForEach(sveDove, id: \.id) { dova in
                        MojeSureUD(idZajednickeDove: dova.id, dova: dova.dovaTxt, brojLajkova: dova.broj_Lajkova, lajkovanjeAction: lajkujDovu, getDoveAction: getDoveAction)
                    }
                }
            }
        }
    ...
    func getDove() async {
        ...
        guard let url = URL(string: UrlHelper.baseUrl + "/Korisnik/get-dove-by-id?korisnikId=\(id)") else {
...
struct PrikaziMojeDove_Previews: PreviewProvider {
